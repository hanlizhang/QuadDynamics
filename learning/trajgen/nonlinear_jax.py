...
def modify_reference(
    ...
    times = np.concatenate(
        [np.linspace(0, ts[i + 1] - ts[i], numsteps // num_seg) for i in range(num_seg)]
    )
    ...
    for i in range(p):
        ...
        # coeff0[i, :, :] = coeff0[i, :, ::-1]
    print("coeff0_copy first 10 after flipping: ", coeff0_copy[0, 0, :])
    print("coeff0_copy second segment first 10 after flipping: ", coeff0_copy[0, 1, :])

    print("coeff0_copy.ravel's first 8", coeff0_copy.ravel()[:8])"""
