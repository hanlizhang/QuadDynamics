...
def simulate(
    ...
    state[0]["wind"] = wind_profile.update(
        0, state[0]["x"]
    ...
    mocap_measurements.append(
        mocap.measurement(state[-1], with_noise=True, with_artifacts=False)
    )
    if use_mocap:
        # In this case the controller will use the motion capture estimate of the pose and twist for control.
        control = [
            sanitize_control_dic(
                controller.update(time[-1], mocap_measurements[-1], flat[-1])
            )
        ]
    else:
        control = [
            sanitize_control_dic(controller.update(time[-1], state[-1], flat[-1]))
        ]
    ...
    state_estimate.append(
        estimator.step(state[0], control[0], imu_measurements[0], mocap_measurements[0])
    )

    exit_status = None

    while True:
        exit_status = exit_status or safety_exit(
            world, safety_margin, state[-1], flat[-1], control[-1]
        )
        ...
        state[-1]["wind"] = wind_profile.update(
            time[-1], state[-1]["x"]
        )  # TODO: Move this to inside the vehicle.
        state.append(vehicle.step(state[-1], control[-1], t_step))
        flat.append(sanitize_trajectory_dic(trajectory.update(time[-1])))
        mocap_measurements.append(
            mocap.measurement(
                state[-1], with_noise=True, with_artifacts=mocap.with_artifacts
            )
        )
        state_estimate.append(
            estimator.step(
                state[-1], control[-1], imu_measurements[-1], mocap_measurements[-1]
            )
        )
        if use_mocap:
            control.append(
...
def traj_end_exit(initial_state, trajectory, using_vio=False):
    ...
    rotf = Rotation.from_rotvec(
        yawf * np.array([0, 0, 1])
    )  # create rotation object that describes yaw
    ...
    def exit_fn(time, state):
        ...
        if using_vio:
            ...
            if (
                time >= min_time
                and norm(state["x"] - xf) < 0.02
                and norm(state["v"]) <= 0.03
                and angle <= 0.02
            ):
                return ExitStatus.COMPLETE
        return None
...
def sanitize_control_dic(control_dic):
    ...
    """
    control_dic["cmd_motor_speeds"] = np.asarray(
        control_dic["cmd_motor_speeds"], np.float64
    ).ravel()
    control_dic["cmd_moment"] = np.asarray(
        control_dic["cmd_moment"], np.float64
    ).ravel()
...
def sanitize_trajectory_dic(trajectory_dic):
    ...
    trajectory_dic["x_dddot"] = np.asarray(
        trajectory_dic["x_dddot"], np.float64
    ).ravel()
    trajectory_dic["x_ddddot"] = np.asarray(
        trajectory_dic["x_ddddot"], np.float64
    ).ravel()

    trajectory_dic["u"] = np.asarray(trajectory_dic["u"], np.float64).ravel()
