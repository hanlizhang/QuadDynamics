...
class CircularTraj(object):
    ...
    def __init__(
        ...
        if plane == "XY" or plane == "YZ" or plane == "XZ":
            ...
            print(
                "CircularTraj Error: incorrect specification of plane. Must be 'XY', 'YZ', or 'XZ' "
            )
            sys.exit(1)
        ...
        if direction == "CW" or direction == "CCW":
            ...
            print(
                "CircularTraj Error: incorrect specification of direction. Must be 'CW' or 'CCW' "
            )
            sys.exit(1)
    ...
    def update(self, t):
        ...
        if self.plane == "XY":
            x = np.array(
                [
                    self.cx + self.radius * np.cos(self.sign * self.omega * t),
            ...
            x_dot = np.array(
                [
                    ...
                    * self.sign
                    * self.omega
                    * np.sin(self.sign * self.omega * t),
                    self.radius
                    * self.sign
                    * self.omega
                    * np.cos(self.sign * self.omega * t),
                    0,
                ]
            )
            x_ddot = np.array(
        ...
        if self.yaw_bool:
            yaw = np.pi / 4 * np.sin(np.pi * t) * 0.1
            yaw_dot = np.pi * np.pi / 4 * np.cos(np.pi * t) * 0.1
            yaw_ddot = -np.pi * np.pi * np.pi / 4 * np.sin(np.pi * t) * 0.1
...
class ThreeDCircularTraj(object):
    ...
    def update(self, t):
        ...
        x = np.array(
            [
                self.cx + self.radius[0] * np.cos(self.omega[0] * t),
        ...
        x_dot = np.array(
            [
                -self.radius[0] * self.omega[0] * np.sin(self.omega[0] * t),
        ...
        x_ddot = np.array(
            [
                -self.radius[0] * (self.omega[0] ** 2) * np.cos(self.omega[0] * t),
        ...
        x_dddot = np.array(
            [
                self.radius[0] * (self.omega[0] ** 3) * np.sin(self.omega[0] * t),
        ...
        x_ddddot = np.array(
            [
                self.radius[0] * (self.omega[0] ** 4) * np.cos(self.omega[0] * t),
